...
impl DevicePathParser for MediaParser {
    fn parse(&self, sub_type: u8, data: &[u8]) -> Result<String> {
        match sub_type {
            0x01 => hard_drive(data),
            // 0x02 => cdrom(data),
            0x03 => media_vendor(data),
            0x04 => file_path(data),
            // 0x05 => media_protocol(data),
            // 0x06 => piwg_firmware_file(data),
            // 0x07 => piwg_firmware_volume(data),
            // 0x08 => relative_offset_range(data),
            // 0x09 => ram_disk(data),
            _ => {
...
fn media_vendor(data: &[u8]) -> Result<String> {
    if data.len() < 16 {
        bail!("Vendor media data is too short");
    }
    let guid = &data[0..16];
    let vendor_data = &data[16..];
...
fn hard_drive(data: &[u8]) -> Result<String> {
    ...
    if partition_number != 0 {
        r = format!(",{:#x},{:#x}", partition_start_lba, partition_size_lba)
    }

    Ok(format!("HD({},{},{}{})", partition_number, prefix, sig, r))
}
...
mod tests {
    ...
    #[case(
        1,
        "0f000000002800000000000000500300000000005e27f1007553d5439eb5de2add4c99320202",
        "HD(15,GPT,00F1275E-5375-43D5-9EB5-DE2ADD4C9932,0x2800,0x35000)"
    )]
    #[case(
        1,
        "0100000000080000000000000000100000000000e9a896cbdcade74bb68e69b6d1ea59cb0202",
        "HD(1,GPT,CB96A8E9-ADDC-4BE7-B68E-69B6D1EA59CB,0x800,0x100000)"
    )]
    #[case(
        1,
        "00000000000000000000000000000000000000000000000000000000431202a0000000000101",
        "HD(0,MBR,0x00000000)"
    )]
    #[case(
        1,
        "01000000000800000000000000E02E0000000000431202a000000000431202a0000000000101",
        "HD(1,MBR,0xa0021243,0x800,0x2ee000)"
    )]
    #[case(
        1,
        "0100000022000000000000000000710200000000009ae315d21d00108d7f00a0c92408fc0202",
        "HD(1,GPT,15E39A00-1DD2-1000-8D7F-00A0C92408FC,0x22,0x2710000)"
    )]
    #[case(
        3,
        "f8d1c555cd04b5468a20e56cbb3052d0",
        "VenMedia(55C5D1F8-04CD-46B5-8A20-E56CBB3052D0,)"
    )]
    #[case(
        3,
        "72f728144ab61e44b8c39ebdd7f893c7",
        "VenMedia(1428F772-B64A-441E-B8C3-9EBDD7F893C7,)"
    )]
    #[case(4, "6b00650072006e0065006c000000", "File(kernel)")]
    #[case(
        4,
        "5c004500460049005c0042004f004f0054005c0042004f004f0054005800360034002e004500460049000000",
        "File(\\EFI\\BOOT\\BOOTX64.EFI)"
    )]
    #[case(
        4,
        "5c004500460049005c0042004f004f0054005c00660062007800360034002e006500660069000000",
        "File(\\EFI\\BOOT\\fbx64.efi)"
    )]
    #[case(4, "5c004500460049005c007500620075006e00740075005c007300680069006d007800360034002e006500660069000000", "File(\\EFI\\ubuntu\\shimx64.efi)")]
    #[case(4, "5c004500460049005c007500620075006e00740075005c0067007200750062007800360034002e006500660069000000", "File(\\EFI\\ubuntu\\grubx64.efi)")]
